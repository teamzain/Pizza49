...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $summarySQL = "SELECT COUNT(*) as sales_count, SUM(grand_total) as daily_total,
                   SUM(JSON_LENGTH(sale_quantity)) as total_quantity
                   FROM sale
                   WHERE DATE(invoice_date) = ?";
    
    $summaryStmt = $conn->prepare($summarySQL);
    $summaryStmt->bind_param("s", $searchDate);
    $summaryStmt->execute();
    ...
    if ($row = $summaryResult->fetch_assoc()) {
        ...
        $totalQuantity = $row['sale_quantity'] ?? 0;
        $averageSaleValue = $salesCount > 0 ? $dailyTotal / $salesCount : 0;
    }
    ...
    $detailStmt->bind_param("s", $searchDate);
    $detailStmt->execute();
...
<head>
    ...
    <link href='https://unpkg.com/boxicons@2.1.4/css/boxicons.min.css' rel='stylesheet'>
    <link rel="stylesheet" href="assets/reportstyle.css">
    <title>Pizza 49 - Daily Sales Report</title>
    <style>
        @media print {
            .no-print {
...
<?php include 'topbar.php'; ?>
    ...
    <div class="container">
        ...
        <form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
            <input type="date" name="search_date" required value="<?php echo $searchDate; ?>" class="no-print">
        ...
        <?php if ($_SERVER["REQUEST_METHOD"] == "POST"): ?>
            <?php if ($salesCount > 0): ?>
                <h2>Sales Summary for <?php echo date('F d, Y', strtotime($searchDate)); ?></h2>
                <div class="summary-cards">
                    <div class="summary-card">
                        <h3>Total Sales</h3>
                        <p>Rs <?php echo number_format($dailyTotal, 2); ?></p>
                    </div>
                    <div class="summary-card">
            ...
            <?php else: ?>
                <p class="no-results">No sales recorded for <?php echo date('F d, Y', strtotime($searchDate)); ?></p>
            <?php endif; ?>
        <?php endif; ?>
    </div>
            </main>
