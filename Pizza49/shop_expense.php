...
$employeeQuery = "SELECT staff_id, staff_name FROM staff";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "INSERT INTO expenses (date, expense_type, amount, description, employee_name) VALUES (?, ?, ?, ?, ?)";

    // Create a prepared statement
    $stmt = $conn->prepare($sql);

    // Bind parameters to the prepared statement
    $stmt->bind_param("ssdss", $date, $expenseType, $amount, $description, $employeeName);
...
<?php include 'topbar.php'; ?>
            ...
            if ($expensesResult) {
                if ($expensesResult->num_rows > 0) {
                    ...
                    while ($expense = $expensesResult->fetch_assoc()) {
                        ...
                        echo "<td data-label='Expense Type'>" . $expense['employee_name'] . "</td>";
    ...
    <select id="employeeName" name="employeeName" >
        ...
        <?php foreach ($employees as $employee): ?>
            <option value="<?php echo $employee['staff_name']; ?>">
                <?php echo $employee['staff_name']; ?>
            </option>
        <?php endforeach; ?>
    </select>
