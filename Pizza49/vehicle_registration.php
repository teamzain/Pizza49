...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // If supplier_id is provided, update the vehicle
    if (!empty($supplier_id)) {
        $sql = "UPDATE vehicle SET car_name=?, car_number=? WHERE id=?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ssi", $supplier_name, $phone_number, $supplier_id);
        
        if ($stmt->execute()) {
            // Use JavaScript to handle alert and redirect
            echo "<script>alert('Vehicle updated successfully!'); window.location.href = '" . $_SERVER['PHP_SELF'] . "';</script>";
        ...
        // Insert a new vehicle if supplier_id is not provided
        $sql = "INSERT INTO vehicle (car_name, car_number) VALUES (?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ss", $supplier_name, $phone_number);
        
        if ($stmt->execute()) {
            // Use JavaScript to handle alert and redirect
            echo "<script>alert('Vehicle added successfully!'); window.location.href = '" . $_SERVER['PHP_SELF'] . "';</script>";
            exit();
        } else {
...
<button class="btn" id="openModalBtn">Add New Vehicle</button>
...
<div id="supplierModal" class="modal">
    <div class="modal-content">
        ...
        <form class="supplier-form" action="vehicle_registration.php" method="POST">
            <label for="supplier_name">Vehicle Name:</label>
            <input type="text" id="supplier_name" name="supplier_name" required>
            <label for="phone_number">Vehicle Number:</label>
...
<div id="editsupplierModal" class="modal">
    <div class="modal-content">
        ...
        <!-- <input type="hidden" id="supplier_id" name="supplier_id"> -->
            ...
            <label for="edit_supplier_name">Vehicle Name:</label>
            <input type="text" id="edit_supplier_name" name="supplier_name" required>

            <label for="edit_phone_number">Vehicle Number:</label>
...
<table>
    ...
    if ($result->num_rows > 0) {
        while($row = $result->fetch_assoc()) {
            ...
            echo "<td>" . $row["id"] . "</td>";
            echo "<td class='supplier_name'>" . $row["car_name"] . "</td>";
            echo "<td class='phone_number'>" . $row["car_number"] . "</td>";
...
function deletesupplier(id) {
    if (confirm('Are you sure you want to delete this Vehicle Data?')) {
        // Corrected path with forward slashes
        window.location.href = 'Action/Delete/delete_vehicle.php?id=' + id;
    }
}
</script>
